...
export function AchievementsPopup({ isOpen, onClose, sets }: AchievementsPopupProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 dark:bg-black/90 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-900 rounded-lg max-w-xl w-full max-h-[85vh] overflow-hidden border border-gray-200 dark:border-gray-700/50 shadow-2xl">
        {/* Header */}
        <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700/50">
          ...
          <div className="mt-4">
            <div className="flex justify-between text-xs mb-1.5">
              ...
              <span className="text-gray-600 dark:text-gray-400">{completedCount}/{achievements.length}</span>
            </div>
            <div className="h-1.5 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
              <div
                className="h-full bg-amber-500 dark:bg-white/80 transition-all duration-500"
          ...
          <div className="grid grid-cols-3 gap-3 mt-4 text-center">
            <div className="bg-gray-100 dark:bg-gray-800/50 rounded-md py-2">
              <div className="text-xl font-semibold text-gray-900 dark:text-white">{stats.uniqueCount}</div>
              <div className="text-[10px] text-gray-500 uppercase tracking-wide">Loại</div>
            </div>
            <div className="bg-gray-100 dark:bg-gray-800/50 rounded-md py-2">
              <div className="text-xl font-semibold text-gray-900 dark:text-white">{stats.totalOpened}</div>
              <div className="text-[10px] text-gray-500 uppercase tracking-wide">Đã mở</div>
            </div>
            <div className="bg-gray-100 dark:bg-gray-800/50 rounded-md py-2">
              <div className="text-xl font-semibold text-gray-900 dark:text-white">{completedCount}</div>
                ...
                <div
                  ...
                  className={`p-3 rounded-md border transition-colors ${
                    achievement.isCompleted
                      ? "bg-amber-50 dark:bg-gray-800/50 border-amber-200 dark:border-gray-700"
                      : "bg-transparent border-gray-200 dark:border-gray-800"
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <Icon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${achievement.isCompleted ? "text-amber-600 dark:text-white" : "text-gray-400 dark:text-gray-600"}`} />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className={`text-sm font-medium ${achievement.isCompleted ? "text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-300"}`}>
                          {achievement.name}
                        </h3>
                        ...
                        <div className="flex-1 h-1 bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-300 ${
                              achievement.isCompleted ? "bg-amber-500 dark:bg-white" : "bg-gray-300 dark:bg-gray-600"
                            }`}
